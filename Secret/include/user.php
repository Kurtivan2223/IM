...
class User
{
    ...
    public static function login()
    {
        ...
        if(get_config('md5_password'))
        {
            ...
            $query->execute(array($_POST['useroremail'], md5($_POST['password'])));
        }
        else
        {
            if(str_contains($_POST['useroremail'], '@'))
            {
                ...
                $query = Database::$connection->prepare("SELECT ID, Username, Email, Fname, Lname FROM Account WHERE Email = :email AND Password = :password");
            }
            else
            {
                $query = Database::$connection->prepare("SELECT ID, Username, Email, Fname, Lname FROM Account WHERE Username = :user AND Password = :password");
            }
        ...
        else
        {
            ...
            success_msg('Successfully Logged in. You will be redirected in 5 seconds.');
            header("refresh:5; http://localhost/user/index.php");
    ...
    public static function register()
    {
        ...
        if (!preg_match('/^[a-zA-Z0-9_]+$/', $_POST['username'])) {
            error_msg('Username should have atleast 1 lower or uppercase letter and 1 Numerical.');
            return false;
        }
        ...
        success_msg('Registration successful! You will be redirected in 5 seconds.');
        header("refresh:5; url=http://localhost/login.php");
    }
